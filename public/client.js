(() => {
  ...
  // window.supabase is the CDN library namespace; sb is our client instance.
  let sb = null;
  let channel = null;
  let myId = null;
  let room = null;
  ...
  async function handleSignal(from, data) {
    ...
    if (!pc) {
      // We didn't have a connection yet (we're the non-initiator) — create one now.
  ...
  // Best-effort logging to Supabase — never blocks or breaks the call if it fails.
  ...
  function connectSignaling() {
    ...
    channel.on('presence', { event: 'sync' }, () => {
      ...
      peerIds.forEach((peerId) => {
        if (!peers.has(peerId)) {
          const initiator = myId < peerId; // deterministic tie-break so only one side offers
  ...
  async function joinCall(roomOverride) {
    const value = (roomOverride || roomInput.value).trim();
    if (!value) {
  ...
  createBtn.addEventListener('click', () => joinCall(generateRoomCode()));
