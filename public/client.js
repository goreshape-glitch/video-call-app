(() => {
  ...
  function handleSignedIn(user) {
    ...
    myId = user.id;
    myEmail = user.email;
    if (!nameInput.value.trim() && myEmail) {
      nameInput.value = myEmail.split('@')[0];
    }
    if (authScreen.hidden === false || verifyScreen.hidden === false) {
      showScreen(welcomeScreen);
    }
  }
  ...
  sendCodeBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      showAuthError(err.message || 'Could not send the link. Please try again.');
    } finally {
  ...
  resendBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      showVerifyError(err.message || 'Could not resend the link.');
    } finally {
      setTimeout(() => {
